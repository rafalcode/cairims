...
#define NUMHL 256 // number of horizontal lines in centre zone (for all image) .. must be odd!
#define NUMVL 256 // number of horizontal lines in centre zone (for all image)
#define WID 640 
#define HEI 480 
...
int main (int argc, char *argv[])
{
    ...
    // hor bars.
    float *vcoords=calloc(NUMHL, sizeof(float));
    int vcentre = HEI/2;
    ...
    // ver bars.
    float *xcoords=malloc(NUMVL, sizeof(float));
    ...
    cairo_set_line_width (cr, 0.5); /* thinnest really possible */
