...
void gethcol(float val, float *rgb) // get a heat colour.
{
  ...
  // Two colours, a and b: aR is the red chanell of a, etc.
...
{
    if(argc!=5) {
        printf("lou2. four args 1) height of rect 2) width of rect 3) nrows 4( ncols.");
        exit(EXIT_FAILURE);
    }
    ...
    int i, nij, nij2, j;
    int lmar =40, rmar =10, tmar =60, bmar =30;
    ...
    unsigned char unevr=0; // uneven rows.
    if(nrow%2)
        unevr=1;
    int totv=nrow*ncol;
    ...
    for(i=0;i<nr2;++i) {
        for(j=0;j<nc2;++j) {
            ...
            tbu[nij].sty=cp.sty - bf2 - rvsz - unevr*(bf2 + rvsz/2) - i*(bframe+rvsz);
    ...
    if(unevr) {
        for(j=0;j<ncol;++j) {
            nij = j;
            nij2 = ncol*nr2+ nij;
            tbu[nij].stx=cp.stx - nc2*(bframe+rhsz) + bf2 + j*(bframe+rhsz);
            tbu[nij].sty=cp.sty - rvsz/2;
            cairo_set_source_rgba(cr, rgba[3*nij2], rgba[3*nij2+1], rgba[3*nij2+2], 0.8);
            cairo_rectangle (cr, tbu[nij].stx, tbu[nij].sty, rhsz, rvsz);
            cairo_fill(cr);
            sprintf(nstr, "%2.1f%%", 100*vala[nij2]);
    ...
    for(i=0;i<nr2;++i) {
        for(j=0;j<nc2;++j) {
            nij = i*nc2+j;
            nij2 = unevr*ncol + ncol*nr2+ (nc2)*(i+1) + nij;
            tbu[nij].stx=cp.stx + bf2 + j*(bframe+rhsz);
            tbu[nij].sty=cp.sty + bf2 + unevr*(bf2 + rvsz/2) + i*(bframe+rvsz);
            cairo_set_source_rgba(cr, rgba[3*nij2], rgba[3*nij2+1], rgba[3*nij2+2], 0.8);
            cairo_rectangle (cr, tbu[nij].stx, tbu[nij].sty, rhsz, rvsz);
            cairo_fill(cr);
            sprintf(nstr, "%2.1f%%", 100*vala[nij2]);
    ...
    for(i=0;i<nr2;++i) {
        for(j=0;j<nc2;++j) {
            nij = i*nc2+j;
            nij2 = unevr*ncol + ncol*nr2 + i*ncol+(nc2-j-1);
    ...
    cairo_surface_write_to_png (surface, "lou2.png");
    free(tbu);
    free(rgba);
    free(vala);
    cairo_destroy (cr);
