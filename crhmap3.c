...
#define IMW 1024
#define IMH 768
#define LSKIP 1 // how many lines to skip at top of input file (i.e. =1 if there' a header of one line).
...
{
    if(argc!=4) {
        printf("crhmap0. Outputs an sample plate arrangement, in heatmap style, where color gives percetnage of called SNPs.\n");
    ...
    int ncol=NCOL;
    int nrow = (aawc->numl-1)/ncol; // auto calc or rows.
    int totv=nrow*ncol;
    int restant = aawc->numl-1-totv;
    int rtotv; // real
    if(restant) {
        rtotv = totv + restant;
        totv += ncol;
    ...
    printf("real totv= %i totv=%i\n", rtotv, totv); 
    float *vala=malloc(rtotv*sizeof(float));
    for(i=0;i<rtotv;++i) {
        vala[i] = atof(aawc->aaw[i+1]->aw[2]->w);
        printf("%2.2f ", vala[i]);
    }
    printf("\n"); 
    ...
    */
    ...
    pla_t *tbu=calloc(rtotv, sizeof(pla_t)); /* plus y */
    for(i=0;i<nrow;++i) {
          for(j=0;j<ncol;++j) {
              nij = ncol*i+j;
              if(nij>=rtotv)
                  break; // break out of outer loop not nec.
              ...
              printf("nij=%i \n", nij); 
              cairo_text_extents(cr, aawc->aaw[nij+LSKIP]->aw[0]->w, &te); // first column has name
              cairo_move_to (cr, rmp[0] - te.x_bearing - te.width/2, rmp[1] - fe.descent + 1+ fe.height);
              cairo_show_text(cr, aawc->aaw[nij+LSKIP]->aw[0]->w);
          }
    }

    cairo_surface_write_to_png (surface, "crhmap3.png");
    free(tbu);
    free(rgba);
    free(vala);
    free_aawc(&aawc);
